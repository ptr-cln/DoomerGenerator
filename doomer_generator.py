...
YOUTUBE_UPLOAD_SCOPES = ["https://www.googleapis.com/auth/youtube.force-ssl"]
VIDEO_FRAME_WIDTH = 1920
VIDEO_FRAME_HEIGHT = 1080
DOOMER_OVERLAY_WIDTH = 878  # 760 * 1.1 * 1.05 (15% larger total)
DOOMER_OVERLAY_HEIGHT = 1132  # 980 * 1.1 * 1.05 (15% larger total)
DOOMER_OVERLAY_LEFT = 0  # Aligned to left edge
...
class UploadSettings:
    ...
    # when privacy_status starts with "Multi-day". Not persisted.
...
def _build_smart_tags(title: str, mood: str | None = None) -> list[str]:
    """Generate high-quality SEO tags using sophisticated pattern matching and genre detection.

    Args:
        title: Video title
        mood: Optional mood phrase generated by AI (e.g. "midnight city lights")
    """
    ...
    # 2. Extract tags from AI-generated mood (if provided)
    mood_keywords = []
    if mood:
        # Extract individual words from mood phrase
        # Example: "midnight city lights" → ["midnight", "city", "lights"]
        mood_words = mood.lower().split()

        # Map mood words to relevant tags
        mood_tag_mapping = {
            "midnight": ["midnight", "late night", "3am vibes"],
            "night": ["night", "nighttime", "late night"],
            "city": ["city", "urban", "cityscape"],
            "lights": ["city lights", "neon", "neon lights"],
            "neon": ["neon", "neon lights", "cyberpunk"],
            "rain": ["rain", "rainy", "rainy night"],
            "rainy": ["rainy", "rain", "rainy vibes"],
            "empty": ["empty", "loneliness", "isolation"],
            "lonely": ["loneliness", "lonely", "isolation"],
            "loneliness": ["loneliness", "lonely", "sad"],
            "silent": ["silence", "quiet", "peaceful"],
            "silence": ["silence", "quiet", "still"],
            "winter": ["winter", "cold", "snow"],
            "cold": ["cold", "winter", "freezing"],
            "drive": ["driving", "night drive", "late night drive"],
            "driving": ["driving", "drive", "car ride"],
            "street": ["streets", "urban", "city streets"],
            "streets": ["streets", "street", "urban"],
            "dark": ["dark", "darkness", "noir"],
            "fog": ["fog", "foggy", "mist"],
            "smoke": ["smoke", "smoking", "cigarette"],
            "3am": ["3am", "late night", "midnight"],
        }

        # Extract relevant tags based on mood words
        for word in mood_words:
            if word in mood_tag_mapping:
                mood_keywords.extend(mood_tag_mapping[word][:2])  # Add top 2 tags per word

        # Also add the full mood phrase as a tag if it's short enough
        if len(mood) <= 25:
            mood_keywords.insert(0, mood)

    # Add mood-derived tags early (high priority)
    if mood_keywords:
        tags.extend(mood_keywords[:6])  # Limit to top 6 mood-derived tags

    # 3. Detect and add genre-specific tags
    genre_tags_added = []
    ...
    # 4. Add artist and song name from title
    ...
    # 5. Add keyword-based tags
    for token in keyword_tokens[:8]:
        tags.append(_sanitize_tag(token))

    # 6. Add mood tags (select 4-5) - only if no AI mood was provided
    if not mood_keywords:
        tags.extend(mood_tags[:5])

    # 7. Add vibe tags (select 3-4)
    tags.extend(vibe_tags[:4])

    # 8. Add activity tags (select 2-3)
    tags.extend(activity_tags[:3])

    # 9. Add keyword variations for SEO
...
def _compose_youtube_tags(
    ...
    if settings.smart_tags_enabled:
        ...
        if ai_tags:
            ...
            # Pass mood to smart tags for better tag generation
            fallback_tags = _build_smart_tags(title, mood=mood)
...
class DoomerGeneratorApp:
    ...
    def _build_upload_tab(self, parent: ttk.Frame) -> None:
        ...
        ttk.Combobox(
            ...
            values=["private", "unlisted", "public", "scheduled", self._t("upload_privacy_multiday")],
            state="readonly",
            width=20,
        ...
        # Button that shows "Select date..." or the selected date
        self.youtube_date_btn = ttk.Button(
            self.youtube_schedule_frame,
            text=self._t("upload_multiday_select_date"),
        ...
        self.youtube_multiday_label = ttk.Label(options_box, text=self._t("upload_multiday_label_scheduling"))
        self.youtube_multiday_btn = ttk.Button(
            options_box,
            text=self._t("upload_multiday_btn_configure"),
    ...
    def _open_multiday_scheduling_popup(self) -> None:
        ...
        popup = tk.Toplevel(self.root)
        popup.title(self._t("upload_multiday_popup_title"))
        popup.transient(self.root)
        popup.grab_set()
        ...
        def _add_row():
            ...
            # Video label
            label = ttk.Label(row_frame, text=self._t("upload_multiday_video_label").format(number=row_index + 1), width=10)
            label.pack(side=tk.LEFT, padx=5)

            # Date button
            date_var = tk.StringVar(value=self._t("upload_multiday_select_date"))
        ...
        def _remove_row(row_dict: dict):
            ...
            if row_dict in row_widgets:
                ...
                for i, row in enumerate(row_widgets):
                    row["label"].configure(text=self._t("upload_multiday_video_label").format(number=i + 1))
        ...
        def _select_date_for_row(row_dict: dict):
            ...
            cal_popup.title(self._t("upload_label_publish_at"))
            ...
            ttk.Button(btn_frame, text=self._t("btn_ok"), command=_confirm_date).pack(side=tk.LEFT, padx=5)
            ttk.Button(btn_frame, text=self._t("upload_multiday_btn_cancel"), command=cal_popup.destroy).pack(side=tk.LEFT, padx=5)
        ...
        def _save_and_close():
            ...
            if not row_widgets:
                messagebox.showerror(
                    self._t("upload_multiday_error_no_configs_title"),
                    self._t("upload_multiday_error_no_configs")
                )
                return
            ...
            for i, row in enumerate(row_widgets):
                ...
                # Validate date
                if not date_str or date_str == select_date_text:
                    messagebox.showerror(
                        self._t("upload_multiday_error_title"),
                        self._t("upload_multiday_error_no_date").format(number=i + 1)
                    )
                    return
                ...
                try:
                    ...
                    messagebox.showerror(
                        self._t("upload_multiday_error_title"),
                        self._t("upload_multiday_error_invalid_date").format(number=i + 1)
                    )
                    return
                ...
                if not hour_str or not minute_str:
                    messagebox.showerror(
                        self._t("upload_multiday_error_title"),
                        self._t("upload_multiday_error_no_time").format(number=i + 1)
                    )
                    return
                ...
                try:
                    ...
                    messagebox.showerror(
                        self._t("upload_multiday_error_title"),
                        self._t("upload_multiday_error_invalid_time").format(number=i + 1)
                    )
                    return
            ...
            for i, (date_obj, hour, minute) in enumerate(configs):
                ...
                if dt <= now:
                    messagebox.showerror(
                        self._t("upload_multiday_error_title"),
                        self._t("upload_multiday_error_past_date").format(number=i + 1)
                    )
                    return
        ...
        if self.multiday_schedule_configs:
            self._log(self._t("upload_multiday_log_prepopulate").format(count=len(self.multiday_schedule_configs)))
            ...
            self._log(self._t("upload_multiday_log_no_previous"))

        # Bottom buttons
        bottom_buttons = ttk.Frame(popup)
        bottom_buttons.pack(side=tk.BOTTOM, pady=10)

        ttk.Button(bottom_buttons, text=self._t("upload_multiday_btn_save"), command=_save_and_close).pack(side=tk.LEFT, padx=5)
        ttk.Button(bottom_buttons, text=self._t("upload_multiday_btn_cancel"), command=popup.destroy).pack(side=tk.LEFT, padx=5)
    ...
    def _update_schedule_visibility(self) -> None:
        ...
        multiday_text = self._t("upload_privacy_multiday")

        if status == "scheduled":
            # Clear date/time fields - user MUST set them manually
            self.youtube_selected_date = None
            self.youtube_date_btn.configure(text=self._t("upload_multiday_select_date"))
    ...
    def _start_youtube_upload(self) -> None:
        ...
        elif self.youtube_privacy_var.get().strip() == self._t("upload_privacy_multiday"):
